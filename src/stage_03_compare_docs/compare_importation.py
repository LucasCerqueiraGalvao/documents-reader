...
def main() -> None:
    ...
    ap.add_argument(
        "--input",
        required=True,
        help="Pasta do Stage 02 importation (com *_fields.json)",
    )
    ap.add_argument(
        "--output", required=True, help="Pasta de saída do Stage 03 (comparação)"
    )
    args = ap.parse_args()

    run_stage_03_comparison(
        in_dir=Path(args.input), out_dir=Path(args.output), verbose=True
    )
...
def run_stage_03_comparison(
    ...
    files = sorted(
        [p for p in in_dir.glob("*_fields.json") if p.name != "_stage02_summary.json"]
    )
    if not files:
        return {
            "processed_count": 0,
