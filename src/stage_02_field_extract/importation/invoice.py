...
RE_COUNTRY_ACQ = re.compile(
    r"(?is)\bCOUNTRY\s+OF\s+ACQUISITION\b\s*[:\-]?\s*([A-Z][A-Z ]{2,})\b"
)
RE_COUNTRY_PROV = re.compile(
    r"(?is)\bCOUNTRY\s+OF\s+PROVENANCE\b\s*[:\-]?\s*([A-Z][A-Z ]{2,})\b"
)
...
def extract_invoice_fields(text: str):
    ...
    fields["shipper_name"] = build_field(
        bool(shipper), True, shipper, [ev] if ev else [], "heuristic_regex"
    )
