...
def extract_pdf_text(
    pdf_path: Path, ocr_lang: str, ocr_dpi: int, min_chars: int
) -> Dict[str, Any]:
    ...
    for i in range(doc.page_count):
        ...
        if len(direct) >= min_chars:
            pages.append(
                PageExtraction(
                    page=i + 1, method="direct", text_chars=len(direct), text=direct
                )
            )
            continue
        ...
        if tesseract_path is None:
            ...
            warnings.append(
                f"{pdf_path.name}: pÃ¡gina {i+1} precisa OCR, mas tesseract nÃ£o estÃ¡ disponÃ­vel."
            )
            pages.append(
                PageExtraction(
                    page=i + 1,
        ...
        try:
            ...
            pages.append(
                PageExtraction(
                    page=i + 1, method="ocr", text_chars=len(ocr_txt), text=ocr_txt
                )
            )
            ...
            pages.append(
                PageExtraction(
                    page=i + 1, method="ocr_error", text_chars=0, text="", note=str(e)
                )
            )
...
def run_stage_01_extraction(
    ...
    if not in_dir.exists():
        raise FileNotFoundError(f"Pasta de entrada não existe: {in_dir}")
    ...
    for pdf in pdfs:
        ...
        ocr_missing = sum(
            1 for p in payload["pages"] if p["method"] == "ocr_unavailable"
        )
        ...
        if verbose:
            print(
                f"OK -> {pdf.stem}_extracted.txt/.json | direct={direct_pages} | ocr={ocr_pages} | ocr_unavailable={ocr_missing} | ocr_error={ocr_error}"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage 01 - Extract text from PDFs (direct + OCR fallback)."
    )
    parser.add_argument(
        "--in", dest="in_dir", required=True, help="Pasta com PDFs de entrada"
    )
    parser.add_argument(
        "--out", dest="out_dir", required=True, help="Pasta de saÃ­da (txt/json)"
    )
    parser.add_argument(
        "--lang", default="eng+por", help="Idiomas do OCR (ex: eng, por, eng+por)"
    )
    parser.add_argument(
        "--dpi", type=int, default=300, help="DPI para render no OCR (ex: 300)"
    )
    parser.add_argument(
        "--min-chars",
        type=int,
        default=80,
        help="MÃ­nimo de chars para considerar texto 'direto'",
    )
